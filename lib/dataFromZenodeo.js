...
const log = (fn, msg) => debug(`lib/dataFromZenodeo:${fn}`)(msg);
...
function sqlRunner({sql, runparams, fastify, returnRows = 'none'}) {
    ...
    try {
        ...
        console.log('ERROR: lib/dataFromZenodeo.js > sqlRunner()');
        console.log(sql);
...
function getDataFromZenodeo ({ request, resource, fastify }) {
    ...
    if (queries.dropTmp) {
        ...
        //
        log(self, `createTmp: ${queries.createTmp}`);
        ...
        log(self, `createIndex: ${queries.createIndex}`);
        ...
        // There is no count query, but there will be a full query if it 
        // includes a resourceId
