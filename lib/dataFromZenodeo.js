...
function sqlRunner(fastify, sql, params, mode='none') {
    ...
    try {
        ...
        if (expandedParams) {
            result = stmt[mode](expandedParams);
        }
        else {
            result = stmt[mode]();
        }
...
function getTreatments({ byGenusSpecies, byBinomens, byChunkIds, byTreatmentAuthors, byTreatments_Ids }) {
    let sql = `
        SELECT 
            tc.id AS chunkId,
