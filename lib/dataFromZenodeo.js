...
function runQuery({ fastify, queryName, queries, runparams, returnRows, debugInfo }) {
    //fastify.zlog.info(fastify.zlog.prefix(), `${queryName}: ${queries[queryName]}`);
...
function getDataFromZenodeo ({ request, resource, fastify, queryType }) {
    ...
    // fastify.zlog.info(fastify.zlog.prefix(), queries);
    // fastify.zlog.info(fastify.zlog.prefix(), runparams);
    ...
    if (queries.full) {
        ...
        if (queries.createTmp) {
            ...
            if (resource === 'images') {
                fastify.zlog.info(fastify.zlog.prefix(), `images: ${queries.full}`);
                res = getSeverKetor({
                ...
                fastify.zlog.info(fastify.zlog.prefix(), `treatments: ${queries.full}`);
                res = getSeverKetor({
                    db: fastify.betterSqlite3, 
                    sql: queries.full,
            ...
            fastify.zlog.info(fastify.zlog.prefix(), `others: ${queries.full}`);
            res = sqlRunner({
                sql: queries.full, 
                runparams, 
                fastify,
