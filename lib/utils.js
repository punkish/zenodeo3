...
function getQueryType(resourceName, request) {
    ...
    function isResourceIdQuery(resourceName, request) {
        ...
        const isNotNone = resourceId !== 'none';
        const existsInParams = resourceId.name in request.query;

        if (resourceId && isNotNone && existsInParams) {
            return true;
        }
