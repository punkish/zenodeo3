...
const timerFormat = function(t) {
    return `${t[0]}s ${(t[1]/1000000).toFixed(2)}ms`
}
...
const getPattern = (zqltype) => {
    ...
    if (zqltype === 'geolocation') {
        ...
        const operator         = `(?<operator>(eq|ne|starts_with|ends_with|contains))?`;
...
const addExternalDef = (param, dictionary) => {
    const newParam = JSON.parse(JSON.stringify(param.dict.filter(e => e.name === param.name)[0]));

    if (param.alias) {
        newParam.alias = param.alias;
    }

    if (param.joins) {
        newParam.joins = param.joins;
    }

    if (param.isResourceId) {
        newParam.isResourceId = param.isResourceId;
    }
    ...
    dictionary.push(newParam);
}

export { timerFormat, re, getPattern, addExternalDef }
