...
async function normalizeQuestion(fastify, question, raw) {
    try {
        ...
        // Step 1: detect only
        const detectPrompt = `What language is this text written in? Reply with only the BCP-47 language tag, nothing else. Examples: "en", "de", "fr", "es".

TEXT: ${question}`;

        const detectedLanguage = await callLangModel(fastify, detectPrompt);
        const originalLanguage = detectedLanguage.trim().toLowerCase().slice(0, 5); // guard against prose bleed

        if (originalLanguage === 'en') {
            return { englishQuestion: question, originalLanguage: 'en', wasTranslated: false };
        }
