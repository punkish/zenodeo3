...
async function askZaiSomething(fastify, question, reply) {
    ...
    if (!contextResult) {
        ...
        // CHANGED: was `return`, now explicit null for cache check
        return null;
    }
    ...
    // CHANGED: return the full result so the caller can cache it.
    return fullResult;
}
...
async function answerFromZai(fastify, context, question, raw) {
    ...
    try {
        // ── PREVIOUSLY — single-shot (non-streaming) ─────────────────────────
...
async function handleAskZaiSomethingWithCache(
    fastify, 
    resource, 
    request, 
    reply
    ...
    if (request.query.refreshCache) {
        ...
        // ── Cache HIT → plain JSON response, no streaming ────────────────────
        fastify.zlog.info('querying cache');
        const cached = await fastify.cache.get(queryObj);

        if (cached) {
            fastify.zlog.info('cache hit — returning cached result as JSON');

            // reply.send() writes the response and signals Fastify we are done.
            // We do NOT set SSE headers here — it's a normal JSON response.
            reply.send(cached);
            return;
        }
    }
    ...
    fastify.zlog.info('cache miss — streaming and caching result');
...
//
// The plugin expects to drive the response itself, so we pass `reply` down
// to sendEvent/sendToken which now call reply.sse.send() instead of raw.write.
// reply.sse.send() is awaitable and resolves when the frame is flushed.
//
// PREVIOUSLY:
