...
async function cloud_orig(fastify, model, messages, options) {
...
async function cloud(fastify, model, messages, options) {
    const url = `${fastify.zconfig.zai.ollama_cloud_url}/api/chat`;
    fastify.zlog.info(`using cloud model ${model} at url: ${url}`);
