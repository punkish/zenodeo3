 ...
 */
async function getContext(fastify, question) {
    const bigrams = questionToBigramsQuery(question);
    const debugInfo = {};

    // Step 1 — detect binomens and authors
    const { binomens, debug: binoDebug } = detectBigrams(fastify, bigrams);
    ...
    // Step 5 — enrich with images, assemble context
    const context = assembleContext(treatments, question);
    const { 
        treatments: enrichedTreatments, 
        debug: imagesDebug 
    } = addImagesToTreatments(fastify, 'fulltext', treatments);
 ...
 */
function detectBigrams(fastify, bigrams) {
    const sql = `
...
function detectTreatmentAuthors(fastify, bigrams) {
    const sql = `
