...
// getContext
//
// CHANGED: accepts `raw` so it can emit progress events as each step resolves.
// No logic changes — the pipeline order is identical to before.
//
...
async function getContext(fastify, question, raw) {
    ...
    if (binomens.length || treatmentAuthors.length) {
        sendEvent(raw, 'status', {
            step: 'tokens',
            message: `Found ${binomens.length} binomens, ${treatmentAuthors.length} authors`,
        });
        ...
        sendEvent(
            raw, 
            'status', 
            { step: 'tokens', count: candidateTreatments.length }
        );
    }
    ...
    // Step 3 — fallback: generic FTS5
        ...
        sendEvent(raw, 'status', { step: 'fts', message: `identified ${result.length} treatments` });
    }

    if (!candidateTreatments.length) return null;

    // Step 4 — subset vector search
    sendEvent(raw, 'status', { step: 'vector', message: 'Running vector search' });
    ...
    sendEvent(raw, 'status', { step: 'vector', message: `winnowed to ${treatments.length} treatments` });
...
function detectBinomens(fastify, bigrams, raw) {
    sendEvent(raw, 'status', { step: 'binomens', message: 'Detecting binomens' });
    ...
    sendEvent(raw, 'status', { step: 'binomens', message: `found ${binomens.length}` });
...
function detectTreatmentAuthors(fastify, bigrams, raw) {
    sendEvent(raw, 'status', { step: 'authors', message: 'Detecting authors' });
    ...
    sendEvent(raw, 'status', { step: 'authors', message: `found ${treatmentAuthors.length}` });
...
function getTreatmentsByTokens(fastify, binomens, treatmentAuthors) {
    ...
    return {
        result: result ?? [],
        debug: { getTreatmentsByTokens: { sql: formatSql(sql, runparams), runtime } }
    };
}
