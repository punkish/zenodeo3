...
async function getContext(fastify, question, reply) {
    ...
    // Step 3 — fallback: generic FTS5 → candidate treatments
        ...
        const searchSql = `
            SELECT DISTINCT t.id AS treatments_id
            FROM treatmentsFts fts
        ...
        const runparams = { query: question, limit: 5000 };
        const returnRows = 'all';
        const { result, runtime } = sqlRunner(fastify, searchSql, runparams, returnRows);
        fastify.zlog.info(`FTS5 search took ${runtime} ms`);
    ...
    if (candidateTreatments.length) {
        ...
        reply.response.treatments = vectorResults.map(v => ({
...
function identifyTokens(fastify, bigrams, isBinomen = true, reply) {
    ...
    if (result) {
        ...
        tokenType = isBinomen ? 'Binomens' : 'TreatmentAuthors';
        ...
        res = tokens.map(t => `'${t.toLowerCase()}'`);
    }

    fastify.zlog.info(`found ${res.length} ${tokenType} in ${runtime} ms`);
    return res;
}
