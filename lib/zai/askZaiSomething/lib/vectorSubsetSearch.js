...
import { cosineSimilarity } from '../../lib/index.js';
 ...
 */
async function subsetVectorSearch(fastify, question, topK=8, treatments_ids) {
    
    // 1. Compute embedding for the query
    const queryVector = await embed(question);
    const sql = getTreatments({ forSomething: true, byTreatments_Ids: true });
    const runparams = { treatments_ids };
    
    const { result, runtime } = sqlRunner(fastify, sql, runparams, 'all');

    if (!result?.length) {
        fastify.zlog.info(`no vectors found for the treatments`);
        return [];
    }

    fastify.zlog.info(`found ${result.length} vectors in ${runtime} ms`);
...
// Shares the existing memory buffer. 
// Faster (instantaneous "pointer" reference). 
// Modifying this array will change the data in the original buffer.
