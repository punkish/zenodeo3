...
function addImagesToTreatments(fastify, question, removeField, reply) {
    reply.debugInfo.images = {};
    const sql = `
    ...
    reply.response.treatmentsByBinomens.forEach((binomenGroup, bidx) => {
        const { binomen, treatments } = binomenGroup;

        treatments.forEach((treatment, tidx) => {
            delete treatment[removeField];
            const treatments_id = treatment.treatments_id;
            const treatmentId = treatment.treatmentId;
            const runparams = { treatments_id };
            const { result, runtime } = sqlRunner(
                fastify, sql, runparams, returnRows
            );
            
            treatment.images = result;
            reply.debugInfo.images[treatmentId] = { 
