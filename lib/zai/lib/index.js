...
import { sendEvent } from '../askZaiSomething/lib/utils.js';
...
function cleanText(text) {
    ...
    const re_replaceWithSpace = new RegExp(
        punctuations.replaceWithSpace.join('|'), 'g'
    );

    const re_remove = new RegExp(
        punctuations.remove.join('|'), 'g'
    );

    text = text.replace(re_replaceWithSpace, ' ');
    text = text.replace(re_remove, '');
    // text = text.replace(/…/g, '');
    // text = text.replace(/\\/g, '');
    // text = text.replace(/#/g, ' ');
    // text = text.replace(/\. \./g, '.');
...
function addImagesToTreatments(fastify, removeField, treatments, raw) {
    sendEvent(raw, 'status', { step: 'images', message: 'Fetching related images' });
    ...
    sendEvent(raw, 'status', { step: 'images', message: `retrieved ${countOfImages} images` });
