...
function getContext({ fastify, question, response, debugInfo }) {
    ...
    const treatmentsFtsQry = buildTreatmentsFtsQry({ 
        searchTerms, detectedBinomens 
    });
    fastify.zlog.info(`treatmentsFtsQry: ${treatmentsFtsQry}`);

    response.ftsQuery = treatmentsFtsQry;
...
function extractBigrams(searchTerms) {
...
function detectBinomens({ fastify, binomensMatch, debugInfo }) {
    ...
    const runparams = { query: binomensMatch };
   ...
   //const binomens = result.map(r => r.binomen );
    if (result) {
        const detectedBinomens = result.binomen;
