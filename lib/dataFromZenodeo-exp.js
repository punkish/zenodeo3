...
function sqlRunner(fastify, sql, runparams, returnRows='none') {
    const runMode = {'none':'run', 'one':'get', 'all':'all'}[returnRows];

    try {
        let t = process.hrtime();
        const result = runparams
            ? fastify.zqlite.prepare(sql)[runMode](runparams)
            : fastify.zqlite.prepare(sql)[runMode]();
        
        t = process.hrtime(t);
        
        return { result, runtime: utils.ns2ms(t) }
    }
    catch(error) {
        const line = '='.repeat(50);
        console.error(`
${line}
${utils.formatSql(sql, runparams)}
${line}
        `);
        throw error;
    }
...
/**
 * @function uniqLatLon - Remove duplicate lat/lon from loc
 *      https://stackoverflow.com/a/53882732/183692
 */
function uniqLatLon(data) {
    const result= data.reduce((cur, nxt)=> {
        const cond = cur.some(a => a.latitude  === nxt.latitude && a.longitude === nxt.longitude);

        if (!cond) cur.push(nxt);
        return cur;
    },[]);
