...
/**
 * If the query result had already been cached, it would have been sent back 
 * directly by the prehandler hook (see server.js). We have reached here so 
 * that means the query result was not in the cache. Here we run the query, 
 * caching and then returning the results
...
const getCacheKey = (request) => {
    ...
    for (const [param, condition] of Object.entries(paramsToRemove)) {
        if (condition === 'false') {
...
const getTreatmentsWithImages = (fastify) => {
    const q1 = `
        SELECT Count(DISTINCT treatments.id) AS num_of_records
        FROM treatments JOIN treatmentsFts ON treatments.id = treatmentsFts.rowid 
        WHERE treatmentsFts.fulltext MATCH @q
    `;
    ...
    let t = process.hrtime();
    const res = fastify.betterSqlite3.prepare(q1).all(runparams);
    t = process.hrtime(t);
    ...
    if (config.isDebug) {
        ...
        debug.count = { query: q1, runtime }
    }

    if (result.count) {
        const q2 = `
            SELECT 
                treatments.id, treatmentId, treatmentTitle, zenodoDep, 
                treatmentDOI, articleTitle, articleAuthor, 
                journals.journalTitle 
            FROM 
                treatments JOIN 
                treatmentsFts ON treatments.id = treatmentsFts.rowid JOIN 
                journals ON treatments.journals_id = journals.id 
            WHERE 
                treatmentsFts.fulltext MATCH @q 
            LIMIT 30
        `;

        t = process.hrtime();
        const res = fastify.betterSqlite3.prepare(q2).all(runparams);
        t = process.hrtime(t);
        let runtime = utils.timerFormat(t);
        
        //result.records = res;

        if (config.isDebug) {
            debug.full = { query: q2, runtime }
        }

        const q3 = `
            SELECT treatments_id, httpUri, captionText 
            FROM images 
        ...
        t = process.hrtime();
        const res2 = fastify.betterSqlite3.prepare(q3).all(runparams);
        t = process.hrtime(t);
        runtime = utils.timerFormat(t);

        for (const { treatments_id, httpUri, captionText } of res2) {

            // Find a value in an array of objects in Javascript [duplicate]
            // https://stackoverflow.com/a/12462414/183692
            const obj = res.find((o, i) => {
                if (o.id == treatments_id) {
                    if ('images' in res[i]) {
                        res[i].images.push({ treatments_id, httpUri, captionText });
                    }
                    else {
                        res[i].images = [ { treatments_id, httpUri, captionText } ];
                    }
...
const getDataFromZenodeo = async ({ request, resource, fastify }) => {
    ...
    if (resource === 'treatments' && params.cols.includes('httpUri')) {
        return getTreatmentsWithImages(fastify);
    }
