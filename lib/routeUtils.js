...
function routeHandler(resourceName, fastify) {
    ...
    return async function(request, reply) {
        ...
        if (queryType.usesCache) {
            fastify.zlog.info('returning cached response');
            return response;
        }
        else {
            fastify.zlog.info('returning uncached response');
...
function getQueryTypeOrig(request, fastify) {
    ...
    fastify.zlog.info(queryType);
    return queryType
}

async function getCachedQueries({ fastify, resource }) {
    fastify.zlog.info('querying cachedQueries');
