...
const queryDataStore = async ({ request, resource }) => {
    ...
    **/
    ...
    if (sourceOfResource === 'zenodeo') {
        data = await getDataFromZenodeo({ request, resource });
    }
    else if (sourceOfResource === 'metadata') {
        data = await getDataFromMetaStore({ request, resource });
    }
    // else {
    //     data = await getDataFromZenodo({ request, resource });
    // }
...
const _sqlRunner = function(sql, runparams) {
    try {
        ...
        const res = db.conn.prepare(sql).all(runparams);
        t = process.hrtime(t);
...
SELECT Count(*) AS num_of_records FROM images JOIN treatments ON images.treatments_id = treatments.id WHERE treatments."checkinTime" BETWEEN strftime('%s', '2020-12-27') * 1000 AND strftime('%s', '2022-12-27') * 1000;

SELECT Count(*) FROM treatments WHERE treatments."checkinTime" BETWEEN strftime('%s', '2010-12-27') * 1000 AND strftime('%s', '2011-12-27') * 1000;

{ from: '2010-12-27', to: '2011-12-27' }
