...
const routeHandler = (resourceName, fastify) => async (request) => {
    ...
    if (config.cache.on) {
        ...
        const foo = async (request, resourceName, fastify, cache=null) => {
        ...
        if (request.query.refreshCache) {
            ...
            const response = foo(request, resourceName, fastify, cache);
            return response;
        }
        else {
...
const _sqlRunner = function(sql, runparams, fastify) {
    try {
        ...
        const res = fastify.betterSqlite3.prepare(sql).all(runparams);
        t = process.hrtime(t);
