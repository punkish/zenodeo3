...
const routeFactory = (resourceName) => {
    ...
    return async function route(fastify, opts) {
...
const getDataFromZenodeo = async ({ request, resource }) => {
    ...
    **/
    ...
    debug.countQuery = { query: queries.main.count, runparams, runtime }
    ...
    **/
    if (result.count) {
        if (queries.main.full) {
            ...
            result.records = res;
            debug.fullQuery = { query: queries.main.full, runparams, runtime }
        }
        ...
        if (queries.related) {
            ...
            for (let [relatedRecord, sql] of Object.entries(queries.related)) { 
                ...
                debug.relatedRecords[relatedRecord] = { query: sql.full, runparams, runtime }
            }
        }
        ...
        if (queries.facets) {
            ...
            for (let [facet, sql] of Object.entries(queries.facets)) {
                ...
                debug.facets[facet] = { query: sql, runparams, runtime }
            }
        }
    }
...
const getDataFromZenodo = async ({ request, resource }) => {
    ...
    **/
    ...
    try {
        ...
        //     : await getRequest(qs);
        const json = await getRequestGet(qs);
