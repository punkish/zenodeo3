...
const getConstraint = (resource, col, val) => {
    ...
    if (typeof(val) === 'string') {
        ...
        if (res) {
            ...
            //
            // There was a match, so there is a zql present in the query. The 
            // zql operator(s) and the operand(s) are in `res.groups`. We 
            // construct the constraints based on the zqltype
            //
            const g = res.groups;
