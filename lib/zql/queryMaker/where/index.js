...
export function getWhere(resource, request) {
    ...
    if (resource.resourceId) {
        const { name, selname } = resource.resourceId;
    ...
    const resourceParams = ddutils.getParams(resource.name);
...
const getConstraint = (resource, request, col, val) => {
            ...
            if (g.operator || g.preglob || g.postglob || g.operator1 || g.operator2) {
                ...
                // publicationDate is stored as a string '2010-12-27'
                    ...
                    r = text({ 
                        ...
                        operator,
                        isZai: request.queryType.type === 'zai' ? true : false
                    });
