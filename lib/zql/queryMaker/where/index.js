...
const getWhere = ({ 
    ...
    // If it is a 'zai' query, we process it and don't need to go any further
    // if (queryType.type === 'zai') {
        
    //     for (const [key, val] in Object.entries(params)) {
    //         const left = key;
    //         const right = val;
    //         const operator = '=';
    //         constraints.push(`${left} ${operator} @${right} COLLATE NOCASE`);
    //         runparams[right] = val;
    ...
    // If we reached here, the query is not a 'zai' query. So we examine
    ...
    resourceParams
        ...
        .forEach(col => {
            ...
            // are returned 
            const res = getConstraint(resource, resourceParams, col, val, queryType);
...
const getConstraint = (resource, resourceParams, col, val, queryType) => {
    ...
    if (typeof(val) === 'boolean') {
        ...
        const operator = utils._zops[col.defaultOp];
        ...
        if (res) {
            ...
            if (g.operator || g.preglob || g.postglob || g.operator1 || g.operator2) {
                ...
                // publicationDate is stored as a string '2010-12-27'
                    ...
                    const operator = utils._zops[zop]; 
                    ...
                    r = text({ 
                        col, 
                        val: g.operand1 || g.operand2, 
                        operator,
                        isZai: queryType.type === 'zai' ? true : false
                    });
