...
const geolocation = (g) => {
    ...
    const constraint = {
        bind: `materialCitations.validGeo = 1 AND materialCitationsRtree.minX BETWEEN @min_lng AND @max_lng AND materialCitationsRtree.minY BETWEEN @min_lat AND @max_lat`,

        vals: `materialCitations.validGeo = 1 AND materialCitationsRtree.minX BETWEEN ${coords[0]} AND ${coords[2]} AND materialCitationsRtree.minY BETWEEN ${coords[1]} AND ${coords[3]}`
    };
