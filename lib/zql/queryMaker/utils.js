 ...
 */
const validate = function({ resource, params, getQueryStringSchema }) {

    if (!getQueryStringSchema) {
        getQueryStringSchema = ddutils.getQueryStringSchema(resource)
    }

    const schema = {
        type: 'object',
        additionalProperties: false,
        properties: getQueryStringSchema
    };
 ...
 * @param {string} resource - name of the resource.
 */
const queryCache = (resource, resourceParams) => {
    ...
    if (!(resource in CACHE._defaultOps)) {
        ...
        const queryableParams = resourceParams
