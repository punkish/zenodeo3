...
const getFullSqlBare = ({
    ...
    clauses.push(`SELECT ${cols.join(', ')} FROM ${tables.join(' ')}`);
...
const getFullSqlResourceId = ({ cols, tables, constraints }) => {
    return `SELECT ${cols.join(', ')} FROM ${tables.join(' ')} WHERE ${constraints.join(' AND ')}`;
}
...
const getCountSqlWithoutConstraints = ({ tables }) => getCountSqlBare({ tables });

const getDropTmpSql = () => `DROP TABLE IF EXISTS tmp`;
...
const getCountSqlNormal = ({ tables, constraints, resourceId }) => {
    const clauses = [];
