...
const normalQueries = ({ 
    ...
    if (params.yearlyCounts) {
        ...
        const count = tables.length > 1
            ? getCountSqlNormal({ tables: [ 'tmp' ], constraints, resourceId })
            : getCountSqlNormal({ tables: [ 'tmp' ], constraints });
        ...
        const count = tables.length > 1
            ? getCountSqlNormal({ tables, constraints, resourceId })
            : getCountSqlNormal({ tables, constraints });
