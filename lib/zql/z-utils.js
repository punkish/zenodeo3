...
const _getConstraint = (resource, key, val) => {
    ...
    const c = ddUtils.getWhere(resource, key, 'query');
    ...
    if (typeof(val) === 'string') {
        ...
        if (res) {
            ...
            if (zqltype === 'geolocation') {
                ...
                if (zop === 'between') {
                    ...
                    constraint.push(`${key} ${operator} @date`)
                    runparam.date = new Date(date).getTime()
                }
         ...
         */
            ...
            if (zop === 'eq') {
                const left = ddUtils.getSelname(resource, key);
                constraint.push(`${left} ${operator} @${key}`);
                runparam[key] = val;
            }
            else if (zop === 'match') {
                ...
                const left = `LOWER(${ddUtils.getSelname(resource, key)})`
...
const getTables = (resource, params) => {
     ...
     * clause. We call the former the joins.select and the latter 
     * the joins.query
     */
...
const getConstraints = ({resource, params}) => {
    ...
    const constraints = [];
    // params.cols.forEach(col => {
    //     const constraint = ddUtils.getWhere(resource, col, 'select');
    //     if (constraint) {
    ...
    if (resourceId.name in params) {
        constraints.push(`${resourceId.selname} = '${params[resourceId.name]}'`);
    ...
    // remove duplicates from constraints and return
    // https://stackoverflow.com/a/15868720
    return {
...
const getLimitAndOffset = (params) => {
    return {
        limit: params.size,
        offset: ((params.page - 1) * params.size) + 1
    }
}
