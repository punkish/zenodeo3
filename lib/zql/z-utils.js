            ...
            if (res) {
                ...
                if (key === 'geolocation') {
                    ...
                    if (zop === 'within') {
                        ...
                        coords = [ g.min_lng, g.min_lat, g.max_lng, g.max_lat ];
                    }

                    constraint = `materialsCitations.latitude ${operator} @min_lat AND @max_lat AND materialsCitations.longitude ${operator} @min_lng AND @max_lng`;
                    
                    runparam.min_lng = coords.min_lng;
                    runparam.min_lat = coords.min_lat;
                    runparam.max_lng = coords.max_lng;
                    runparam.max_lat = coords.max_lat;
                }
                else {
                    if (zqltype === 'date') {
                        const left = ddutils.getWhere(resource, key);
                        if (zop === 'between') {                    
                            constraint = `${left} ${operator} strftime('%s', @from) * 1000 AND strftime('%s', @to) * 1000`;

                            runparam.from = formatDate(g.from);
                            runparam.to = formatDate(g.to);
                        }
                        else if (zop === 'eq' || zop === 'since' || zop === 'until') {
                            constraint = `${left} ${operator} strftime('%s', @date) * 1000`;
...
const getWhere = ({ resource, params }) => {
    ...
    **/
    ...
    if (resourceIdName in params) {
        //constraints.push(`${resourceId} = '${params[resourceIdName]}'`);
    ...
    **/
    ...
    Object.keys(params)
        ...
        .forEach(param => {
             ...
             * runparam <object> are returned 
            **/
            const { constraint, runparam } = _getConstraint(resource, key, val);
