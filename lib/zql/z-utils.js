...
const getSelect = (resource, params) => {
    ...
    const columns = [ `${resource}.${resourceId.name}` ];
...
const getWhere = ({ resource, params }) => {
    ...
    if (resourceId.name in params) {
        const resourceIdSelname = ddUtils.getSelect(resource, resourceId.name);
        constraints.push(`${resourceIdSelname} = '${params[resourceId.name]}'`);
