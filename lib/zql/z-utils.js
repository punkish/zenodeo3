            ...
            if (res) {
                ...
                if (key === 'geolocation') {
                    ...
                    if (zqltype === 'date') {
                        ...
                        if (zop === 'between') {                    
                            constraint = `CAST(strftime('%s', ${left}) AS INT) ${operator} CAST(strftime('%s', @from) AS INT) * 1000 AND CAST(strftime('%s', @to) AS INT) * 1000`;
                            runparams.from = formatDate(g.from);
                            runparams.to = formatDate(g.to);
                        }
                        else if (zop === 'eq' || zop === 'since' || zop === 'until') {
                            constraint = `CAST(strftime('%s', ${left}) AS INT) ${operator} CAST(strftime('%s', @date) AS INT) * 1000`;
