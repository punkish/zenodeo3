...
const _castanet = (input) => `CAST(strftime('%s', ${input}) AS INT)`;
 ...
 * | updateTime      | int    | sssssssssssss |
 * | checkinTime     | int    | sssssssssssss |
 * | journalYear     | text   | yyyy          |
 * | authorityYear   | text   | yyyy          |
 * | checkinYear     | int    | yyyy          |
 *
 */
const _isDate = (resource, key, zop, g, runparams, operator) => {
    ...
    if (zop === 'between') {                    
        ...
        constraint = `${left} ${operator} CAST(strftime('%s', @date) AS INT)`;
