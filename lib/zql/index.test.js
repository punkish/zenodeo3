...
const searchparams = 'biome=pampas&cols=treatmentId&cols=treatmentTitle&cols=zenodoDep&cols=treatmentDOI&cols=articleTitle&cols=articleAuthor&cols=httpUri&cols=caption&cols=latitude&cols=longitude&yearlyCounts=true';
...
tests.forEach((test, i) => {
    //if (i === 41) {
      tap.test(`test ${i}: ${test.input.searchparams}`, tap => {
        const params = preZql(test.input.searchparams);
        const found = zql({ resource: test.input.resource, params });
        // console.log(found)
        tap.same(found, test.wanted);
