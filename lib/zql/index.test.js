...
const tests = [
    ...
    // 10
    {
      input: {
        resource: 'images',
        searchparams: 'articleDOI=10.11646/zootaxa.5284.3.7&yearlyCounts=true&page=1&size=30&cols=treatmentId&cols=treatmentTitle&cols=zenodoDep'
      },
      wanted: {
        queries: {
          dropTmp: 'DROP TABLE IF EXISTS tmp',
    ...
    // 19
    {
      ...
      wanted: {
        queries: {
          dropTmp: 'DROP TABLE IF EXISTS tmp',
          createTmp: `CREATE TEMP TABLE tmp AS SELECT images.id AS images_id, images.httpUri, images.captionText, treatments.treatmentId, treatments.treatmentTitle, treatments.treatmentDOI, treatments.zenodoDep, treatments.articleTitle, treatments.articleAuthor, images.treatments_id FROM images JOIN treatments ON images.treatments_id = treatments.id JOIN materialCitations ON images.treatments_id = materialCitations.treatments_id JOIN ${geoSchema}.biome_synonyms ON materialCitations.biomes_id = ${geoSchema}.biome_synonyms.biomes_id WHERE biome_synonyms.biome_synonym LIKE @biome ORDER BY images_id ASC',
          createIndex: 'CREATE INDEX ix_tmp_treatments_id ON tmp(treatments_id)`,
          count: 'SELECT Count(DISTINCT images_id) AS num_of_records FROM tmp',
          full: 'SELECT * FROM tmp LIMIT 30 OFFSET 0',
          yearlyCounts: "SELECT strftime( '%Y', treatments.checkinTime/1000, 'unixepoch' ) AS year, Count(DISTINCT images.id) AS num_of_images, Count(DISTINCT treatments.id) AS num_of_treatments, Count(DISTINCT species.id) AS num_of_species, Count(DISTINCT journals.id) AS num_of_journals FROM images JOIN treatments ON images.treatments_id = treatments.id JOIN species ON treatments.species_id = species.id JOIN journals ON treatments.journals_id = journals.id WHERE treatments.id IN (SELECT treatments_id FROM tmp) GROUP BY year ORDER BY year ASC"
    ...
    // 41
    {
      input: { resource: 'materialCitations', searchparams: 'validGeo=true&cols=latitude&cols=longitude&page=1&size=1000' }
    },

    // 43
    {
      input: { resource: 'binomens', searchparams: 'cols=&groupby=&page=1&size=30&sortby=binomen%3aasc&stats=false&termFreq=false&yearlyCounts=false' }
    }
];

import Zlogger from '../../../zlogger/index.js';
const zlog = new Zlogger({ 
    //name: 'ZQL', 
    // level, 
    // transports, 
    // mode, 
...
tests.forEach((test, i) => {
    if (dontSkip.includes(i)) {
      //tap.test(`test ${i}`, tap => {
        console.log(`Test ${i}`);
        console.log(`query: ${test.input.resource}/${test.input.searchparams}`)
        const params = preZql(test.input);
...
// const searchparams = 'id=85425&cols=httpUri';
