    ...
    normalQueries: [
        ...
        // 4. without yearlyCounts, multiple tables
        {
            ...
            wanted: {
                ...
                full: "SELECT images.id, images.httpUri, images.figureDoi, images.captionText, images.treatments_id FROM images JOIN treatments ON images.treatments_id = treatments.id JOIN treatmentsFts ON treatments.id = treatmentsFts.rowid WHERE treatmentsFts.fulltext MATCH @q AND treatments.treatmentTitle LIKE @treatmentTitle ORDER BY images.id ASC LIMIT 30 OFFSET 0",
...
Object.keys(testGroups).forEach((testGroupName) => {
    ...
    tests.forEach((test, i) => {
        tap.test(`${testGroupName} ${i}`, tap => {
            const found = mainQueries(test.input);
