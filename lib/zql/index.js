...
const log = require('../../lib/utils')('ZQL')
        ...
        if (key.indexOf('$') === -1) {
            ...
            if (i === null || i === -1) {
                ...
                if (key === 'geoloc_operator') {
                    const res = ops('geolocation', params.geoloc_operator, params.geolocation, colNames)
...
const facetQueries = ({ resource, params }) => {
    ...
    columns.forEach(c => {
        ...
        facets[c.name] = `SELECT ${c.name}, Count(${c.name}) AS count FROM ${tables.join(' JOIN ')} WHERE ${c.name} != "" GROUP BY ${c.name}`
    })
