...
const regularQueries = (resource, params) => {
    ...
    const constraints_tmp = [];
    params.cols.forEach(col => {
        const c = ddUtils.getWhere(resource, col);
        if (c) {
            constraints_tmp.push(c);
        }
    });
    ...
    Object.keys(params)
        ...
        .forEach(p => {
            ...
            const {cstr, runparam} = zUtils.getConstraint(resource, p, params[p]);
            // if (typeof(c) === 'undefined') {
            //     console.log(resource, p, params[p])
            // }
    ...
    // remove duplicates from tables and constraints
    // https://stackoverflow.com/a/15868720
    const tables = [ ...new Set(tables_tmp) ];
    const constraints = [ ...new Set(constraints_tmp) ];
...
//     resource: 'treatments',
//     params: { 
//         location: 'containedIn({lowerLeft: {lat: -40.00, lng: -120}, upperRight: {lat: 23, lng: 6.564}})',
//         cols: ['latitude','longitude']
//     }
// })
