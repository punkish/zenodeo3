...
const getSql = (obj, sqlType) => {
    const { columns, tables, constraints, sortorder, limit, offset } = obj;
...
const zql = ({ resource, params }) => {
    ...
    **/
    const count = getSql({ columns, tables, constraints }, 'count');
    ...
    if (params.cols.length) {
        ...
        **/
        if (resourceIdName in params) {
            full = getSql({ columns, tables, constraints }, 'full');
        }
        ...
        **/
            ...
            full = getSql(
                { columns, tables, constraints, sortorder, limit, offset },
                'full'
            );
        }
    ...
    **/
    if (params.stats && resource === 'treatments') {
        result.queries.stats = statsQueries({ tables, constraints });
    }
...
const statsQueries = ({ tables, constraints }) => {
    ...
    for (let i = 0, j = entities.length; i < j; i++) {
        ...
        const sql = `SELECT checkInYear, Count (DISTINCT ${tb}.${pk}) AS num FROM ${t.join(' ')} WHERE ${c.join(' AND ')} GROUP BY 1`;
    ...
    **/
    ...
    stats.locations = `SELECT materialsCitations.country, Count(materialsCitations.materialsCitationId) AS num FROM ${t.join(' ')}  WHERE ${c.join(' AND ')} GROUP BY country ORDER BY num DESC`;
        ...
        if (sql) {
            if (typeof(sql) === 'string') {
                ...
                for (let [k, v] of Object.entries(sql)) {
                    console.log(k);
                    console.log('-'.repeat(50));
                    console.log(format(v, sqlformatterOpts));
                    console.log('='.repeat(50));
                }
            }
