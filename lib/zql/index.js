...
const facetQueries = (resource) => {
    const facets = {};
    const table = ddu.tableFromResource(resource);
    const tables = [ table ];
    
    const columns = ddu.getFacetCols(resource);
...
const statsQueries = ({ tables, constraints }) => {
    const entities = [
        { tb: 'tr.treatments',         pk: 'treatmentId'         },
        { tb: 'mc.materialsCitations', pk: 'materialsCitationId' },
        { tb: 'fc.figureCitations',    pk: 'figureCitationId'    },
        { tb: 'tc.treatmentCitations', pk: 'treatmentCitationId' },
        { tb: 'bc.bibRefCitations',    pk: 'bibRefCitationId'    },
    ];
    ...
    for (let i = 0, j = entities.length; i < j; i++) {
        ...
        if (tb !== 'tr.treatments') {
            t.push(`JOIN ${tb} ON ${tb}.treatmentId = tr.treatments.treatmentId`);
        }

        const obj = {
            columns: [ 
                'tr.treatments.checkInYear', 
    ...
    if (t.filter(e => e.search(/materialsCitations/) === -1)) {
        ...
        if (t.filter(e => e.search(/treatments/) > -1)) {
            t.push('JOIN mc.materialsCitations ON tr.treatments.treatmentId = mc.materialsCitations.treatmentId');
        }
        else {
            ...
            t.push(`JOIN tr.treatments ON ${resourceId} = tr.treatments.treatmentId`);
            t.push('JOIN mc.materialsCitations ON tr.treatments.treatmentId = mc.materialsCitations.treatmentId');
        }
    }
    ...
    const obj = {
        columns: [
            'mc.materialsCitations.country',
            'Count(mc.materialsCitations.materialsCitationId) AS num'
        ],
...
const test = ({ resource, searchparams }, format = false, results = false) => {
    ...
    const formatter = (sql) => format 
        ? formatDialect(sql, sqlformatterOpts) 
        : sql;

    const printQuery = ({ header, body, sql, runparams, results }) => {
        r(header);
        ...
        if (sql) {
            if (typeof(sql) === 'string') {
                ...
                if (results) {
                    dbres = db.prepare(sql).all(runparams);
                    console.log(`records found: ${JSON.stringify(dbres, null, 4)}`, '\n');
    
                    if (res.queries.full) {
                        dbres = db.prepare(res.queries.full).all(runparams);
                        console.log(`records found: ${JSON.stringify(dbres, null, 4)}`, '\n');
                    }
                }
                ...
                for (let [k, stmt] of Object.entries(sql)) {
                    console.log(k);
                    console.log('-'.repeat(50));
                    console.log(formatter(stmt));
                    console.log('='.repeat(50), '\n');

                    if (results) {
                        dbres = db.prepare(stmt).all(runparams);
                        console.log(`records found: ${JSON.stringify(dbres, null, 4)}`, '\n');
                    }
                }
            }
    ...
    printQuery({ header: 'count query', sql: res.queries.count, runparams: res.runparams, results });

    printQuery({ header: 'full query', sql: res.queries.full, runparams: res.runparams, results });
    ...
    if (res.queries.related) {
        ...
        for (let [key, val] of Object.entries(res.queries.related)) {
            ...
            printQuery({ header: 'count query', sql: val.count, runparams: res.runparams, results });
            printQuery({ header: 'full query', sql: val.full, runparams: res.runparams, results });
        }
    }
    ...
    if (res.queries.facets) {
        if (Object.keys(res.queries.facets).length) {
            ...
            for (let [key, val] of Object.entries(res.queries.facets)) {
                printQuery({ header: `queries for: ${key}`, sql: val, runparams: res.runparams, results });
            }
        }
    }
...
node lib/zql/index.js --list=true
...
/*
SELECT
    Count(DISTINCT tr1.treatmentId) AS num_of_records
FROM
    tr.treatments AS tr1
    JOIN tr.ftsTreatments AS tr2 ON tr1.treatments.id = tr2.ftsTreatments.rowid
WHERE
    tr2.ftsTreatments MATCH 'tyrannosaurus'
    AND tr1.authorityName LIKE 'osborn%'

SELECT
    Count(DISTINCT tr.treatments.treatmentId) AS num_of_records
FROM
    tr.treatments 
    JOIN tr.ftsTreatments ON tr.treatmentstreatments.id = tr.ftsTreatments.rowid
WHERE
    tr.ftsTreatments.ftsTreatments MATCH 'tyrannosaurus'
    AND tr.treatments.authorityName LIKE 'osborn%'
*/