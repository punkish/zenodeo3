...
const { getSchema, getQueryableParams, getDefaultCols, getAllCols, getResourceId, columnExists } = require('../../data-dictionary/dd-utils')
...
const selfrom = (resource, params) => {
    ...
    if ('$cols' in params) {
        ...
        for (; i < j; i++) {
            ...
            const column = columnExists(resource, col)
            if (column) {
                cols.push(column)
            }
            else {
