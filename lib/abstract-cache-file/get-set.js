...
get(data[id].key)
    .then((data) => {
        if (data) {
            logger.info('found in cache and still fresh')
            send(JSON.parse(data))
        }
        else {
            logger.info('found in cache but stale')
            logger.info('querying for fresh data')
            const data = query(id)
            ...
            set(data.key, data.value, data.ttl)
                .then(() => {
                    ...
                    logger.info('successfully stored 2')
                    send(payload)
                })
                .catch((error) => {
                    logger.info(error)
    ...
    .catch((error) => {
        ...
        set(data[id].key, data[id].value, data[id].ttl)
            .then(() => {
                ...
                logger.info('successfully stored 1')
                send(payload)
            })
            .catch((error) => {
                logger.info(error)
