...
, ecoregions_id INTEGER, biomes_id INTEGER, realms_id INTEGER);
...
CREATE INDEX ix_images_treatments_id_id ON images(treatments_id, id);
CREATE INDEX ix_materialCitations_lon_lat ON materialCitations(longitude, latitude);
...
CREATE INDEX ix_materialCitations_realms_id ON materialCitations ("realms_id");
...
CREATE INDEX ix_genera_genus ON genera(genus);
CREATE INDEX ix_treatments_treatmentId ON treatments(treatmentId COLLATE NOCASE);
CREATE INDEX ix_treatments_summary ON treatments(summary);
...
CREATE INDEX ix_treatments_hasSpeciesSummary ON treatments(hasSpeciesSummary);
CREATE INDEX ix_treatments_rank_genera_species_summary ON treatments(rank, genera_id, species_id, summary);
...
END;
CREATE VIRTUAL TABLE binomens USING fts5(binomen, content=binomen_view, content_rowid=id, tokenize='trigram')
/* binomens(binomen) */;
...
CREATE TRIGGER tr_afterInsertFts AFTER INSERT ON treatments 
    BEGIN
