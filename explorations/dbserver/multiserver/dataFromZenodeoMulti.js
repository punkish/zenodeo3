...
import { getSeverKetor } from './treatmentsData.js';
...
function sqlRunner({sql, runparams, fastify, returnRows = 'none'}) {
    try {
        ...
        if (returnRows === 'none') {
            res = runparams
                ? db.conn.prepare(sql).run(runparams)
                : db.conn.prepare(sql).run();
        }
        else if (returnRows === 'one') {
            res = runparams
                ? db.conn.prepare(sql).get(runparams)
                : db.conn.prepare(sql).get();
        }
        else if (returnRows === 'many') {
            res = runparams
                ? db.conn.prepare(sql).all(runparams)
                : db.conn.prepare(sql).all();
        }
