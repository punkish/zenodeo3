...
const parseOne = function(truebug, treatmentId) {
    ...
    if (s.parse) {
        ...
        if (truebug.run === 'real') {
            const xml = fs.readFileSync(`${truebug.dirs['dump']}/${treatmentId}.xml`, 'utf8')
...
//     return _parse($, 'figureCitation', 'figureCitations', 'figureCitationId', treatmentId);
// };
        ...
        for (let i = 0; i < num; i++) {
            ...
            if (num_of_explosions) {
                for (let i = 0; i < num_of_explosions; i++) {
                    const entry = {}
                    for (let [key, value] of Object.entries(cols_to_explode)) {
                        entry[key] = value ? $(el).attr(value[i]) : $(el).attr(key)
                    }
...
const parseFigureCitations_old = function($, treatmentId) {
    const elements = $('figureCitation');
    ...
    const allCols = getSqlCols('figureCitations')
    //const match_list = ['httpUri', 'captionText']
    ...
    if (num) {
        for (let i = 0; i < num; i++) {
            ...
            const keys = Object.keys(e.attribs)
            ...
            allCols.forEach(col => {
                if (col.cheerio) {
                    ...
                    for (const key in e.attribs) {
                        ...
                        const is_matched = match_list.some((item) => { return key.match(new RegExp('^' + item + '-[0-9]+', 'gi')) })
                        if (is_matched) {
                            entry[key] = e.attribs[key]
                        }
                        else {
                        ...
                        entry['figureCitationId'] = $(e).attr('id') || chance.guid()
                        entry.treatmentId = treatmentId
                        entry.updateVersion = $(e).attr('updateVersion') || ''
                        entry.deleted = deleted
                    }
            ...
            if (entry.figureCitationId === '136E2A751268FFDFD3EB6CB6FE91D88F') {
                console.log(e.attribs)
            }
...
const parse = function(truebug) {
    ...
    if (truebug.source === 'single') {
        const treatmentId = truebug.download.single
