...
const selCountOfTreatments = () => DB.treatments.prepare('SELECT Count(*) AS c FROM treatments')
    .get()
    .c
    ...
    if (s.createTables) {
        ...
        const dbs = {
            ...
            UNIQUE (materialsCitationId, treatmentId)
                ...
                materialsCitations_x_collectionCodes: `CREATE TABLE IF NOT EXISTS materialsCitations_x_collectionCodes ( 
            id INTEGER PRIMARY KEY,
        ...
        for (let db in dbs) {
            ...
            for (let t in tables) {
                log.info(`  - creating table ${b(t)}`)
                if (truebug.run === 'real') DB[db].prepare(tables[t]).run()
            }
        }

        createInsertStatements(truebug)
    }
}
...
const insertData = function(truebug, data) {
    ...
    if (s.insertData) {
        //log.info('inserting data in the db')
