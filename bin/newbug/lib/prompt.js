...
${w('$ node bin/newbug/index.js')}
    ${w('--do    =[parse | counts | archiveUpdates | etl]')}
...
${g('Note 1:')}
${g('─'.repeat(len))}
${g(`- ${underline('all')} options except --do are optional.`)}
${g('- if no options are provided, usage is printed in the terminal.')}
${g('- if --help is used, usage is printed in the terminal.')}
${g('- options not provided are picked up from config.')}
...
${y(`$ index.js --do=counts`)}      ${gr('// get count in each table')}
${y(`$ index.js --do=archiveUpdates`)} ${gr('// updates for all archives')}
${g('─'.repeat(len))}
${g('Note 2: No other options are required for the above --do invocations')}
${g('─'.repeat(len))}

${y(`$ index.js --do=etl`)}            ${gr('// perform the ETL')}

${g('─'.repeat(len))}
${g('Note 3: If --do is not provided, it is picked up from config.')}
${g('─'.repeat(len))}
