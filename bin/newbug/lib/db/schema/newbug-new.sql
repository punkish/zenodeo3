...
/* materialCitationsRtree(id,minX,maxX,minY,maxY,longitude,latitude,treatments_id) */;
CREATE TABLE IF NOT EXISTS "materialCitationsRtree_rowid"(rowid INTEGER PRIMARY KEY,nodeno,a0,a1,a2);
...
/* binomens(binomen) */;
CREATE TABLE IF NOT EXISTS 'binomens_data'(id INTEGER PRIMARY KEY, block BLOB);
CREATE TABLE IF NOT EXISTS 'binomens_idx'(segid, term, pgno, PRIMARY KEY(segid, term)) WITHOUT ROWID;
CREATE TABLE IF NOT EXISTS 'binomens_content'(id INTEGER PRIMARY KEY, c0);
CREATE TABLE IF NOT EXISTS 'binomens_docsize'(id INTEGER PRIMARY KEY, sz BLOB);
CREATE TABLE IF NOT EXISTS 'binomens_config'(k PRIMARY KEY, v) WITHOUT ROWID;
...
CREATE INDEX treatments_journalYear ON treatments(journalYear);
CREATE INDEX treatments_journals_id ON treatments(journals_id);
CREATE TRIGGER treatmentCitations_ai AFTER INSERT ON treatmentCitations 
    BEGIN 
    ...
    END;
CREATE TRIGGER treatmentCitations_ad AFTER DELETE ON treatmentCitations 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'treatmentCitations'; 
    END;
CREATE TRIGGER treatmentAuthors_ai AFTER INSERT ON treatmentAuthors 
    BEGIN 
        INSERT INTO rowcounts (tblname, rows) 
    ...
    END;
CREATE TRIGGER treatmentAuthors_ad AFTER DELETE ON treatmentAuthors 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'treatmentAuthors'; 
    END;
CREATE TRIGGER materialCitations_ai AFTER INSERT ON materialCitations 
    BEGIN 
        INSERT INTO rowcounts (tblname, rows) 
    ...
    END;
CREATE TRIGGER materialCitations_ad AFTER DELETE ON materialCitations 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'materialCitations'; 
    END;
CREATE TRIGGER figureCitations_ad AFTER DELETE ON figureCitations 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'figureCitations'; 
    END;
CREATE TRIGGER bibRefCitations_ai AFTER INSERT ON bibRefCitations 
    BEGIN 
        INSERT INTO rowcounts (tblname, rows) 
    ...
    END;
CREATE TRIGGER bibRefCitations_ad AFTER DELETE ON bibRefCitations 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'bibRefCitations'; 
...
CREATE TRIGGER journals_ad AFTER DELETE ON journals 
    BEGIN 
        ...
        WHERE tblname = 'journals'; 
    END;
CREATE TRIGGER kingdoms_ai AFTER INSERT ON kingdoms 
    BEGIN 
        INSERT INTO rowcounts (tblname, rows) 
        VALUES ('kingdoms', 1) 
        ON CONFLICT(tblname) DO 
    ...
    END;
CREATE TRIGGER kingdoms_ad AFTER DELETE ON kingdoms 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'kingdoms'; 
    END;
CREATE TRIGGER phyla_ai AFTER INSERT ON phyla 
    BEGIN 
        INSERT INTO rowcounts (tblname, rows) 
        VALUES ('phyla', 1) 
        ON CONFLICT(tblname) DO 
            ...
            WHERE tblname = 'phyla'; 
    END;
CREATE TRIGGER phyla_ad AFTER DELETE ON phyla 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'phyla'; 
    END;
CREATE TRIGGER classes_ai AFTER INSERT ON classes 
    BEGIN 
        INSERT INTO rowcounts (tblname, rows) 
        VALUES ('classes', 1) 
        ON CONFLICT(tblname) DO 
    ...
    END;
CREATE TRIGGER classes_ad AFTER DELETE ON classes 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'classes'; 
...
CREATE TRIGGER orders_ad AFTER DELETE ON orders 
    BEGIN 
        ...
        WHERE tblname = 'orders'; 
    END;
CREATE TRIGGER genera_ai AFTER INSERT ON genera 
    BEGIN 
        INSERT INTO rowcounts (tblname, rows) 
        VALUES ('genera', 1) 
        ON CONFLICT(tblname) DO 
    ...
    END;
CREATE TRIGGER genera_ad AFTER DELETE ON genera 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'genera'; 
    END;
CREATE TRIGGER families_ai AFTER INSERT ON families 
    BEGIN 
        INSERT INTO rowcounts (tblname, rows) 
        VALUES ('families', 1) 
        ON CONFLICT(tblname) DO 
    ...
    END;
CREATE TRIGGER families_ad AFTER DELETE ON families 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'families'; 
    END;
CREATE TRIGGER species_ai AFTER INSERT ON species 
    BEGIN 
        INSERT INTO rowcounts (tblname, rows) 
        VALUES ('species', 1) 
        ON CONFLICT(tblname) DO 
    ...
    END;
CREATE TRIGGER species_ad AFTER DELETE ON species 
    BEGIN 
        UPDATE rowcounts 
        SET rows = rows - 1 
        WHERE tblname = 'species'; 
...
CREATE INDEX rowcounts_tblname ON rowcounts(tblname);
...
CREATE TRIGGER treatments_ad AFTER DELETE ON treatments
    BEGIN
        ...
        WHERE tblname = 'treatments';
    END;
CREATE TRIGGER figureCitations_ai AFTER INSERT ON figureCitations 
    BEGIN
